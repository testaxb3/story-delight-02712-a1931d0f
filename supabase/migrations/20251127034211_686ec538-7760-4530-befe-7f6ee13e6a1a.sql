-- Fix current_streak calculation v2: Move date validation to HAVING clause
-- Issue: WHERE filters individual days before grouping, causing only most recent day to count
-- Solution: Use HAVING to validate the entire streak group after grouping

DROP VIEW IF EXISTS leaderboard_cache CASCADE;
DROP VIEW IF EXISTS user_achievements_stats CASCADE;

-- Recreate user_achievements_stats with corrected current_streak logic
...
current_streak_calc AS (
  ...
  -- Validate entire streak group: streak is valid if most recent day is today or yesterday
